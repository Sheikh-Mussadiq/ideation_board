...
const Translate = memo(({ children, textKey }) => {
  const { tSync, t, language, isLoading } = useLanguage();
  ...
  // For small text items, show without loading indicator to avoid UI flicker
  if (!localLoading || String(textToTranslate).length < 20) {
    return <>{translatedText || textToTranslate}</>;
  }
  
  // For longer text that's loading, show a subtle loading indicator
  return (
    <span className="relative">
      <span className="opacity-60">{translatedText || textToTranslate}</span>
      <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gray-300 dark:bg-gray-700 overflow-hidden">
        <span className="absolute h-full w-1/3 bg-design-primaryPurple animate-translateX"></span>
      </span>
    </span>
  );
});
